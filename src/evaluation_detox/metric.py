...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--preds", help="path to predictions of a model", required=True
    )

    parser.add_argument(
        "--cola_classifier_path", default="style_transfer/cola_classifier"
    )
    parser.add_argument("--wieting_model_path", default="im.pt")
    parser.add_argument("--wieting_tokenizer_path", default="sim.sp.30k.model")
    ...
    with open("results.md", "a") as res_file:
        ...
        res_file.writelines(
            f"{name}|{accuracy:.4f}|{emb_sim:.4f}|{avg_sim_by_sent:.4f}|{char_ppl:.4f}|"
