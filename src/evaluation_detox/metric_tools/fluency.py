...
def calc_flair_ppl(preds, aggregate=True):
    ...
    for sent in preds:
        try:
            ...
            print(
                f'Got exception "{e}" when calculating flair perplexity for sentence "{sent}"'
            )
            pp = model.calculate_perplexity(sent + ".")
        flair_ppl.append(pp)
    ...
    with torch.no_grad():
        for sent in tqdm.tqdm(preds):
            ...
            try:
                ...
                print(
                    f'Got exception "{e}" when calculating gpt perplexity for sentence "{sent}" ({input_ids})'
                )
                loss = 100
...
def do_cola_eval(args, preds, soft=False):
    ...
    for i in tqdm.tqdm(
        range(0, len(preds), args.batch_size), total=len(preds) // args.batch_size
    ):
        ...
        batch = collate_tokens(
            [
                cola_roberta.task.source_dictionary.encode_line(
                    "<s> " + sent + " </s>", append_eos=False
                )
        ...
        with torch.no_grad():
            predictions = cola_roberta.predict(
                "sentence_classification_head", batch.long()
            )
